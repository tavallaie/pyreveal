/**
 * Docs homepage flow mesh — adapted for the PyReveal demo title slide.
 * Source: docs/javascripts/home-hero.mjs
 */
import * as THREE from "three";

const CANVAS_ID = "demo-hero-canvas";
const INK = 0x08090c;
...
function isHomeSlide(indices) {
  return indices.h === 0 && (indices.v ?? 0) === 0;
}
...
class FlowMesh3D {
  ...
  renderFrame(time) {
    const t =
      time ?? this.clock.getElapsedTime() * TIME_SCALE;
    this._updateMesh(t);
...
function mountFullPageHero() {
  if (document.getElementById("demo-home-fullpage")) {
    return document.getElementById("demo-home-fullpage");
  }

  const hero = document.querySelector(".demo-home-hero");
  const reveal = document.querySelector(".reveal");
  if (!hero || !reveal) {
    return null;
  }

  hero.id = "demo-home-fullpage";
  reveal.insertBefore(hero, reveal.firstChild);
  return hero;
}

function setHomeActive(enabled) {
  document.body.classList.toggle("demo-home-active", enabled);
  const hero = document.getElementById("demo-home-fullpage");
  if (hero) {
    hero.classList.toggle("is-active", enabled);
    hero.setAttribute("aria-hidden", enabled ? "false" : "true");
  }
}

function needsReinit() {
  if (!active) return true;
  const gl = active.canvas.getContext("webgl2") || active.canvas.getContext("webgl");
  if (!gl) return true;
  return gl.isContextLost();
}

function init() {
  const hero = mountFullPageHero();
  ...
  destroy();
  hero?.classList.remove("no-hero-webgl");

  try {
    active = new FlowMesh3D(canvas);
  } catch (error) {
    console.warn("Demo hero WebGL failed:", error);
    hero?.classList.add("no-hero-webgl");
  }
}
...
function syncHero() {
  ...
  const onHome = isHomeSlide(indices);
  setHomeActive(onHome);

  if (onHome) {
    requestAnimationFrame(() => wakeHero());
  } else {
    active?.pause();
  }
}
...
function bindReveal() {
  if (bound) return;
  bound = true;
  Reveal.on("slidechanged", syncHero);
  Reveal.on("slidetransitionend", syncHero);
  Reveal.on("overviewshown", syncHero);
  Reveal.on("overviewhidden", syncHero);
  Reveal.on("resize", () => {
    if (document.body.classList.contains("demo-home-active")) {
      active?._resize();
      active?.renderFrame();
    }
  });
